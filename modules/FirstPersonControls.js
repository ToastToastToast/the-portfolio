...
class FirstPersonControls extends Controls {
	...
	update(delta) {
		...
		if (this._rotateLeft) this._lon += actualLookSpeed * this.rotationSpeed;
		if (this._rotateRight) this._lon -= actualLookSpeed * this.rotationSpeed;
		if (this._rotateUp) this._lat += actualLookSpeed * this.rotationSpeed;
		if (this._rotateDown) this._lat -= actualLookSpeed * this.rotationSpeed;
	
		this._lat = Math.max(-85, Math.min(85, this._lat));
	
		const phi = MathUtils.degToRad(90 - this._lat);
		const theta = MathUtils.degToRad(this._lon);
